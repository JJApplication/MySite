---
title: 服务器使用denyhosts保护远程登录
date: 2019-08-13 19:47:41
tags:
  - linux
categories:
abstract:
...
最近发现服务器有很多登录失败尝试，每天有个几千次，于是加固一下ssh登录

linux服务器安装`denyhosts`使服务器远程登录更安全

<!--more-->

使用ssh连接服务器，在最近日志里经常看到
`last failed login ***times`

通过查看系统安全日志，发现是一直有ip在恶意暴力破解企图登录服务器，为了服务器更安全同时禁止外部ip的攻击安装`denyhosts`
...
```python
mkdir denyhost && mv DenyHosts-2.6.tar.gz denyhost              #新建存放目录
cd denyhost
...
cp daemon-control-dist daemon-control #主程序
cp denyhosts.cfg-dist denyhosts.cfg #配置文件
chown root daemon-control
chmod 700 daemon-control

ln -s /usr/share/denyhosts/daemon-control /etc/init.d/denyhosts #为服务创建自启软链接
chkconfig denyhosts on #添加服务启动项

/etc/init.d/denyhosts start #启动服务首次需要等待一会
```

默认的配置文件名称为`denyhosts.cfg`所以把原本的复制一份改名

### 自定义

修改`denyhosts.cfg`文件自定义需求

```python
SECURE_LOG = /var/log/secure #安全日志路径
HOSTS_DENY = /etc/hosts.deny #阻止的ip地址写入的文件路径
PURGE_DENY = 1d #日志自动删除时间间隔
BLOCK_SERVICE = sshd #阻止的进程名称

DENY_THRESHOLD_INVALID = 5 #允许不存在用户尝试的次数
DENY_THRESHOLD_VALID = 5 #允许普通用户尝试的次数
DENY_THRESHOLD_ROOT = 5 #允许root用户尝试的次数

HOSTNAME_LOOKUP=YES #是否做域名反解析
ADMIN_EMAIL = denyhosts@163.com #设置管理员的邮箱，出现问题发生邮件

AGE_RESET_VALID=1d #有效用户登录失败的重置时间
AGE_RESET_ROOT=1d #root用户
AGE_RESET_INVALID=10d #无效用户
```
